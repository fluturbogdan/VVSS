...
public class Activity {
	...
	public boolean equals(Object obj) {
		...
		if (act.description.equals(description) && start.equals(act.start)
				&& duration.equals(act.duration) && name.equals(act.name)&& locatie.equals(act.locatie))
			return true;
		return false;
	}
	...
	public static Activity fromString(String line, RepositoryContact repcon) {
		try {
			...
			String locatie = str[4];
			List<Contact> conts = new LinkedList<Contact>();
			for (int i = 5; i < str.length; i++) {
				conts.add(repcon.getByName(str[i]));
			}
			return new Activity(name, start, duration, conts, description,locatie);
		} catch (Exception e) {
			return null;
		}
	}
